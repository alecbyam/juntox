"""Contenu de départ pour le blog, transcrit depuis l'ancien
frontend/lib/blog-data.ts (liste statique) vers le format markdown attendu
par BlogPost.content. Ces 7 articles étaient déjà publiés publiquement avant
le passage au backend — ce fichier ne fait que les faire migrer, sans rien
inventer. Utilisé une seule fois par _seed_blog_posts() dans main.py, qui ne
s'exécute que si la table blog_posts est vide (voir ce fichier).
"""
...
BLOG_SEED = [
    {
        ...
        "excerpt": (
            "Une analyse des facteurs démographiques, économiques et technologiques qui "
            "positionnent l'Afrique comme le prochain centre d'innovation mondiale, et les "
            "secteurs à plus fort potentiel pour la décennie à venir."
        ),
        "category": "Vision",
        "content": """L'Afrique regorge de clichés. On parle de ses ressources naturelles, de ses conflits, de son « potentiel ». Rarement de son avenir technologique. C'est une erreur d'analyse profonde. Les signaux sont là, convergents et puissants : le continent africain est en train de devenir le prochain grand terrain d'innovation mondiale.
...
L'Afrique ne sera pas le prochain hub technologique mondial parce que les conditions seront parfaites. Elle le sera parce que des milliers d'entrepreneurs africains choisiront de construire ici, maintenant, avec ce qu'ils ont — et refuseront d'attendre que les conditions s'améliorent.""",
    },
    {
        ...
        "excerpt": (
            "Comment l'intelligence artificielle transforme déjà les entreprises du continent et "
            "les opportunités concrètes à saisir dans les prochaines années — agriculture, finance, "
            "santé et au-delà."
        ),
...
Notre approche : commencer par comprendre le problème réel, puis choisir la technologie la plus adaptée — pas l'inverse. L'IA n'est pas une fin en soi. Elle est un outil au service d'une mission concrète : moderniser et autonomiser les organisations qui construisent la RDC.""",
    },
    {
        ...
        "excerpt": (
            "Au lieu de copier les modèles existants, nous repartons des fondamentaux pour "
            "construire une entreprise véritablement différente. Retour sur la méthode qui guide "
...
Chez JuntoX, c'est notre mode de pensée par défaut. Pour chaque projet, pour chaque service, pour chaque décision stratégique. L'objectif n'est pas d'être différent pour être différent — c'est de trouver la meilleure réponse possible à chaque question, même si cette réponse ressemble à ce que font les autres.""",
    },
    {
        ...
        "excerpt": (
            "Analyse des bottlenecks logistiques en République Démocratique du Congo — routes, "
            "fleuve Congo, chaîne du froid — et les solutions technologiques concrètes que nous "
            "développons."
        ),
        "category": "Logistique",
        "content": """La République Démocratique du Congo est deux fois et demie la superficie de l'Europe de l'Ouest. Avec 2,3 millions de kilomètres carrés, elle est le deuxième plus grand pays d'Afrique — et l'un des plus mal desservis logistiquement au monde. Ce n'est pas une fatalité. C'est un problème d'ingénierie avec des solutions concrètes.

## L'infrastructure routière : le défi premier
...
## Le fleuve Congo : autoroute naturelle sous-utilisée
...
## La chaîne du froid et la perte alimentaire
...
Notre objectif à moyen terme : réduire de 30 % le coût de transport des entreprises qui utilisent notre plateforme, en optimisant la consolidation de fret et en éliminant les intermédiaires inutiles.""",
    },
    {
        ...
        "excerpt": (
            "Les principes techniques et philosophiques qui guident nos choix architecturaux — "
            "API-first, modularité, scalabilité — pour une plateforme conçue pour durer des décennies."
        ),
        "category": "Technologie",
        "content": """« Construire pour 100 ans » n'est pas une figure de style. C'est une contrainte de conception réelle. Quand vous décidez que votre plateforme doit servir des entreprises, des institutions et des millions d'utilisateurs pendant des décennies — et pas seulement jusqu'au prochain pivot startup — vous prenez des décisions architecturales radicalement différentes.
...
PostgreSQL pour la base de données, hébergé sur Railway. Docker pour la reproductibilité : le code tourne de la même façon sur tous les environnements. Ce ne sont pas des choix à la mode — ce sont des choix raisonnés sur la longévité, la maintenabilité et la performance.
...
Chez JuntoX, nos standards : pas de magic numbers, conventions de nommage strictes, tests unitaires sur les fonctions critiques, et une documentation à jour du système. Ce n'est pas glamour — mais c'est ce qui fait qu'une plateforme reste maintenable après avoir changé d'équipe trois fois.""",
    },
    {
        ...
        "excerpt": (
            "Comment concilier performance financière et impact social mesurable dans le contexte "
            "africain — et pourquoi l'Afrique est le terrain idéal pour cette classe d'actifs."
        ),
        "category": "Investissements",
        "content": """Il y a une idée reçue persistante dans le monde de l'investissement : que le rendement financier et l'impact social sont fondamentalement en tension. Que pour faire du bien, il faut accepter de moins gagner. Cette idée est fausse — et l'Afrique est en train de le prouver.
...
Notre valeur ajoutée n'est pas seulement financière. Nous apportons un réseau, des expertises transversales (technique, stratégique, opérationnelle), et une compréhension profonde du contexte congolais que peu d'investisseurs internationaux possèdent.""",
    },
    {
        ...
        "excerpt": (
            "Retour d'expérience sur l'application du modèle de Simon Sinek à la construction de "
            "notre stratégie d'entreprise — et pourquoi le Why doit être opérationnel, pas décoratif."
        ),
        "category": "Philosophie",
        "content": """Simon Sinek a présenté le Golden Circle pour la première fois en 2009, dans un TED Talk devenu l'un des plus regardés de l'histoire de la plateforme. L'idée est simple : les entreprises qui inspirent commencent par le Pourquoi, pas par le Quoi. Sa mise en pratique, elle, est plus difficile que sa compréhension intellectuelle.

## Le modèle de Simon Sinek
...
## Notre Why, notre How, notre What
...
## Pourquoi ça change tout dans la pratique
